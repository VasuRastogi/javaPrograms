...
class exp1{
    ...
    public static int CalculateSalary(int index){
    ...
    static String findDesignation(String str){
        return switch (str) {
            case "e" -> DesigBlue[0][1];
            case "c" -> DesigBlue[1][1];
            case "k" -> DesigBlue[2][1];
            case "r" -> DesigBlue[3][1];
            case "m" -> DesigBlue[4][1];
            default -> "";
        };
    }
    static String findDAsalary(String str){
        return switch (str) {
            case "e" -> DesigBlue[0][2];
            case "c" -> DesigBlue[1][2];
            case "k" -> DesigBlue[2][2];
            case "r" -> DesigBlue[3][2];
            case "m" -> DesigBlue[4][2];
            default -> "";
        };
    }
        ...
        try{
            for(int i=0; i<=blueArr.length; i++) {
                if (Integer.parseInt(blueArr[i][0]) == Integer.parseInt(exp)){
                    ...
                    result_table_body[4] = String.valueOf(CalculateSalary(i));
                    break;
                }
            }
            for (String s : result_table_head) {
                System.out.print(s + "    ");
            }
            System.out.print("\n\n");
            for (String s : result_table_body) {
                System.out.print(s + "        ");
            }
            System.out.print("\n\n");
